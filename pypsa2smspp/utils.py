...
# the status SMS++ returns when it has solved the problem, i.e. Solver::kOK
# [see ThinComputeInterface.h]; every other one says that the value it wrote
# is not the optimum, and some of them, kLowPrecision in particular, come
# with a value that looks like one
SMSPP_OK_STATUS = 10


def check_solver_status(result, fp_log=None):
    """
    Refuse the answer of SMS++ unless it says it solved the problem.

    pySMSpp calls the run a success whenever the log carries a finite
    objective value, whatever the status of the Solver: an InvestmentBlock
    whose bundle stops on an inexact oracle, for instance, writes a 0 and the
    status 20 (kLowPrecision), and that 0 would otherwise be read as the
    optimum.

    Parameters
    ----------
    result : pysmspp result object
        What `SMSNetwork.optimize()` returned; its `status` carries the status
        of the Solver, written "<status> (Success)" by the wrapper of
        ucblock_solver and "Success (<status>)" by the others.
    fp_log : str or Path, optional
        The log file to point at in the message.

    Raises
    ------
    ValueError
        If the status is not that of a solved problem.
    """
    status = str(getattr(result, "status", ""))
    match = re.search(r"\d+", status)

    if match and int(match.group()) == SMSPP_OK_STATUS:
        return

    objective = getattr(result, "objective_value", None)
    message = (f"SMS++ did not solve the problem: status {status}, objective "
               f"value {objective}")
    if fp_log is not None:
        message += f" [see {fp_log}]"
