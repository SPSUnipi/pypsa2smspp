...
class TransformationConfig:
    ...
    def reset(self):
        ...
        self.BatteryUnitBlock_store_parameters = {
            ...
            # "DeltaRampUp": np.nan,
            # "DeltaRampDown": np.nan,
            "ExtractingBatteryRho": lambda efficiency_dispatch, snapshots_weighting: snapshots_weighting / efficiency_dispatch.iloc[0],
            "StoringBatteryRho": lambda efficiency_store, snapshots_weighting: snapshots_weighting * efficiency_store.iloc[0],
            "StandingBatteryRho": lambda standing_loss, snapshots_weighting: (1 - standing_loss.iloc[0]) ** snapshots_weighting,
            "Demand": 0.0,
        ...
        self.HydroUnitBlock_parameters = {
            ...
            "LinearTerm": lambda efficiency_dispatch, efficiency_store, snapshots_weighting: np.array([efficiency_dispatch.values.max() / snapshots_weighting, 1 / efficiency_store.values.max() / snapshots_weighting if efficiency_store.values.max() != 0 else 0]),
            # "DeltaRampUp": np.nan,
            # "DeltaRampDown": np.nan,
